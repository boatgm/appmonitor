...
import datetime
import os
from crawler.settings import MONGODB,PROJECT_PATH
...
class StatsToMongo(object):
    ...
    def check_new_task(self,spider):
        if spider.name == 'marketcontent':
            if self.db.appmeta.find({"name":"360","lock":None}).count()>10:
                command = "cd "+PROJECT_PATH+"; scrapy crawl marketcontent"
                print command
                #os.system(command)
